  ...
  | _ :: "translate" :: _ ->
      ...
      (match opt "--native-proofs" with
       ...
       | Some npdir ->
           ...
           let proved_order : string list ref = ref [] in
                 ...
                 | Some ast ->
                     ...
                     if (match Hashtbl.find_opt tried i.Manifest.name with
                       ...
                       (match (try Nativeproof.prove ~premises:prem ast with _ -> None) with
                        | Some pf -> Hashtbl.replace np_proved i.Manifest.name pf;
                                     proved_order := i.Manifest.name :: !proved_order; changed := true
                        | None ->
                          match (try Nativeproof.prove_via_rewrites ~premises:prem ast with _ -> None) with
                          | Some pf -> Hashtbl.replace np_proved i.Manifest.name pf;
                                       proved_order := i.Manifest.name :: !proved_order; changed := true
                          | None -> Hashtbl.replace tried i.Manifest.name np)) pubs
           done;
             ...
             let outs = List.filter_map (fun i ->
               ...
               | Some _ ->
                   ...
                   (match Hashtbl.find_opt np_proved i.Manifest.name with
                    ...
                    | Some _ ->
                        incr n_ok;
                        (* emit the self-contained re-proof: the premise-using variant may
                           cite public theorems from a later shard (composition order) *)
...
let guided = List.filter (fun nm -> not (Hashtbl.mem np_self nm)) (List.rev !proved_order) in
           if guided <> [] then begin
             let oc = open_out (Filename.concat npdir "zz_guided.mg") in
             Printf.fprintf oc "// Native proofs guided by recorded proof leaves (docs/DESIGN.md 24.3):\n// rewrite-normalization to True with the leaf lemmas, one congruence step per claim.\n\n";
             List.iter (fun nm ->
               match Hashtbl.find_opt np_proved nm, List.find_opt (fun i -> i.Manifest.name = nm) pubs with
               | Some pf, Some i ->
                   Printf.fprintf oc "// HOL Light: %s / %s (leaf-guided)\nTheorem %s : %s.\n%s\nQed.\n\n"
                     i.Manifest.src_file i.Manifest.source_name nm i.Manifest.statement pf
               | _ -> ()) guided;
             close_out oc
           end;
           Printf.eprintf "native proofs: %d of %d public theorems (%d leaf-guided)\n" !n_ok !n_try (List.length guided));
