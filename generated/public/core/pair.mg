// Generated by hol2mg from HOL Light 433477862bb90b328a593e012e09390e99b2439b, profile core, shard pair.
// Do not edit: regenerate from the exporter, mapping registry, or overrides.
// Statements are admitted imports; none is a new axiom.

// HOL Light: pair.ml:17 / LET_DEF
// Source hash: md5:b500f0ea638d009d27d0410e5045329f
// Status: exact_native
Theorem LET_DEF : forall A B:set, A <> Empty -> B <> Empty -> forall f:set -> set, (forall x :e A, f x :e B) -> forall x :e A, f x = f x.
Admitted.

// HOL Light: pair.ml:20 / LET_END_DEF
// Source hash: md5:3540107cfa54c763381fa257e545baa3
// Status: exact_native
Theorem LET_END_DEF : forall A:set, A <> Empty -> forall t :e A, t = t.
Admitted.

// HOL Light: pair.ml:67 / FST_DEF
// Source hash: md5:085c9267477376ef68995b88baf72064
// Status: transport_required (bridges: choose_in_spec, hol_prod_setprod)
Theorem FST_DEF : forall A B:set, A <> Empty -> B <> Empty -> forall p :e A * B, p 0 = choose_in A (fun x:set => exists y :e B, p = (x,y)).
Admitted.

// HOL Light: pair.ml:70 / SND_DEF
// Source hash: md5:f00cacab114b37c19aba6b06c35bb916
// Status: transport_required (bridges: choose_in_spec, hol_prod_setprod)
Theorem SND_DEF : forall A B:set, A <> Empty -> B <> Empty -> forall p :e A * B, p 1 = choose_in B (fun y:set => exists x :e A, p = (x,y)).
Admitted.

// HOL Light: pair.ml:73 / PAIR_EQ
// Source hash: md5:71770245f4ddd568d85751053a9e5a6e
// Status: transport_required (bridges: hol_prod_setprod)
Theorem PAIR_EQ : forall A B:set, A <> Empty -> B <> Empty -> forall x :e A, forall y :e B, forall a :e A, forall b :e B, (x,y) = (a,b) <-> x = a /\ y = b.
Admitted.

// HOL Light: pair.ml:82 / PAIR_SURJECTIVE
// Source hash: md5:8a85c46beb474e1e63192937ada8908b
// Status: transport_required (bridges: hol_prod_setprod)
Theorem PAIR_SURJECTIVE : forall A B:set, A <> Empty -> B <> Empty -> forall p :e A * B, exists x :e A, exists y :e B, p = (x,y).
Admitted.

// HOL Light: pair.ml:92 / FST
// Source hash: md5:e9c3a4a3164ca2f9e6fad2d535f8ab0f
// Status: transport_required (bridges: hol_prod_setprod)
Theorem FST : forall A B:set, A <> Empty -> B <> Empty -> forall x :e A, forall y :e B, (x,y) 0 = x.
Admitted.

// HOL Light: pair.ml:100 / SND
// Source hash: md5:e4ac88e20ba11cb060769e77dcb122fe
// Status: transport_required (bridges: hol_prod_setprod)
Theorem SND : forall A B:set, A <> Empty -> B <> Empty -> forall x :e A, forall y :e B, (x,y) 1 = y.
Admitted.

// HOL Light: pair.ml:108 / PAIR
// Source hash: md5:23613fb98ddd9bc0b8abe81386bec32f
// Status: transport_required (bridges: hol_prod_setprod)
Theorem PAIR : forall A B:set, A <> Empty -> B <> Empty -> forall x :e A * B, (x 0,x 1) = x.
Admitted.

// HOL Light: pair.ml:115 / pair_INDUCT
// Source hash: md5:77b383354fddaa2478d8ab26c6548c60
// Status: transport_required (bridges: hol_prod_setprod)
Theorem pair_INDUCT : forall A B:set, A <> Empty -> B <> Empty -> forall P:set -> prop, (forall x :e A, forall y :e B, P (x,y)) -> forall p :e A * B, P p.
Admitted.

// HOL Light: pair.ml:121 / pair_RECURSION
// Source hash: md5:d319dfea4248978ad1e1ecfa13aee49a
// Status: transport_required (bridges: hol_prod_setprod)
Theorem pair_RECURSION : forall A B C:set, A <> Empty -> B <> Empty -> C <> Empty -> forall PAIR':set -> set -> set, (forall x :e A, forall x0 :e B, PAIR' x x0 :e C) -> exists fn:set -> set, (forall x :e A * B, fn x :e C) /\ forall a0 :e A, forall a1 :e B, fn (a0,a1) = PAIR' a0 a1.
Admitted.

// HOL Light: pair.ml:187 / CURRY_DEF
// Source hash: md5:cede113a4abd362ecff19b551d8931ed
// Status: transport_required (bridges: hol_prod_setprod)
Theorem CURRY_DEF : forall A B C:set, A <> Empty -> B <> Empty -> C <> Empty -> forall f:set -> set, (forall x :e A * B, f x :e C) -> forall x :e A, forall y :e B, f (x,y) = f (x,y).
Admitted.

// HOL Light: pair.ml:190 / UNCURRY_DEF
// Source hash: md5:3f03a0bbd511648cd075ac39bfcfc4bb
// Status: transport_required (bridges: hol_prod_setprod)
Theorem UNCURRY_DEF : forall A B C:set, A <> Empty -> B <> Empty -> C <> Empty -> forall f:set -> set -> set, (forall x :e A, forall x0 :e B, f x x0 :e C) -> forall x :e A, forall y :e B, f ((x,y) 0) ((x,y) 1) = f x y.
Admitted.

// HOL Light: pair.ml:295 / FORALL_PAIR_THM
// Source hash: md5:259d29f2f6087c2bd6a11a52412f4491
// Status: transport_required (bridges: hol_prod_setprod)
Theorem FORALL_PAIR_THM : forall A B:set, A <> Empty -> B <> Empty -> forall P:set -> prop, (forall p :e A * B, P p) <-> forall p1 :e A, forall p2 :e B, P (p1,p2).
Admitted.

// HOL Light: pair.ml:299 / EXISTS_PAIR_THM
// Source hash: md5:64b789e52ff2b13a976b5610389dc69b
// Status: transport_required (bridges: hol_prod_setprod)
Theorem EXISTS_PAIR_THM : forall A B:set, A <> Empty -> B <> Empty -> forall P:set -> prop, (exists p :e A * B, P p) <-> exists p1 :e A, exists p2 :e B, P (p1,p2).
Admitted.

// HOL Light: pair.ml:326 / FORALL_UNCURRY
// Source hash: md5:3474782c5e2db898cde1d87c3d0177cc
// Status: transport_required (bridges: hol_prod_setprod)
Theorem FORALL_UNCURRY : forall A B C:set, A <> Empty -> B <> Empty -> C <> Empty -> forall P:set -> prop, (forall f :e C :^: B :^: A, P f) <-> forall f:set -> set, (forall x :e A * B, f x :e C) -> P (fun a :e A => fun b :e B => f (a,b)).
Admitted.

// HOL Light: pair.ml:332 / EXISTS_UNCURRY
// Source hash: md5:506bf2c55fa053a1e1ca3b2230e04c52
// Status: transport_required (bridges: hol_prod_setprod)
Theorem EXISTS_UNCURRY : forall A B C:set, A <> Empty -> B <> Empty -> C <> Empty -> forall P:set -> prop, (exists f :e C :^: B :^: A, P f) <-> exists f:set -> set, (forall x :e A * B, f x :e C) /\ P (fun a :e A => fun b :e B => f (a,b)).
Admitted.

// HOL Light: pair.ml:345 / FORALL_UNPAIR_THM
// Source hash: md5:2797620921562307b55cefd5730f67cd
// Status: transport_required (bridges: hol_prod_setprod)
Theorem FORALL_UNPAIR_THM : forall A B:set, A <> Empty -> B <> Empty -> forall P:set -> set -> prop, (forall x :e A, forall y :e B, P x y) <-> forall z :e A * B, P (z 0) (z 1).
Admitted.

// HOL Light: pair.ml:349 / EXISTS_UNPAIR_THM
// Source hash: md5:1887731dff3899ab41ca881b0c28956e
// Status: transport_required (bridges: hol_prod_setprod)
Theorem EXISTS_UNPAIR_THM : forall A B:set, A <> Empty -> B <> Empty -> forall P:set -> set -> prop, (exists x :e A, exists y :e B, P x y) <-> exists z :e A * B, P (z 0) (z 1).
Admitted.

// HOL Light: pair.ml:353 / FORALL_PAIR_FUN_THM
// Source hash: md5:3f44ed9f97562f42b653ea47012379e0
// Status: transport_required (bridges: hol_prod_setprod)
Theorem FORALL_PAIR_FUN_THM : forall A B C:set, A <> Empty -> B <> Empty -> C <> Empty -> forall P:set -> prop, (forall f :e B * C :^: A, P f) <-> forall g:set -> set, (forall x :e A, g x :e B) -> forall h:set -> set, (forall x :e A, h x :e C) -> P (fun a :e A => (g a,h a)).
Admitted.

// HOL Light: pair.ml:359 / EXISTS_PAIR_FUN_THM
// Source hash: md5:5dc28a6866e08e1050ff02cae3d8934d
// Status: transport_required (bridges: hol_prod_setprod)
Theorem EXISTS_PAIR_FUN_THM : forall A B C:set, A <> Empty -> B <> Empty -> C <> Empty -> forall P:set -> prop, (exists f :e B * C :^: A, P f) <-> exists g:set -> set, (forall x :e A, g x :e B) /\ exists h:set -> set, (forall x :e A, h x :e C) /\ P (fun a :e A => (g a,h a)).
Admitted.

// HOL Light: pair.ml:364 / FORALL_UNPAIR_FUN_THM
// Source hash: md5:1a6f72f877498c6aad1d2989bbd1132b
// Status: transport_required (bridges: hol_prod_setprod)
Theorem FORALL_UNPAIR_FUN_THM : forall A B C:set, A <> Empty -> B <> Empty -> C <> Empty -> forall P:set -> set -> prop, (forall f :e B :^: A, forall g :e C :^: A, P f g) <-> forall h:set -> set, (forall x :e A, h x :e B * C) -> P (fun x :e A => h x 0) (fun x :e A => h x 1).
Admitted.

// HOL Light: pair.ml:368 / EXISTS_UNPAIR_FUN_THM
// Source hash: md5:93177ccf6056b9a0f9a555699cb99488
// Status: transport_required (bridges: hol_prod_setprod)
Theorem EXISTS_UNPAIR_FUN_THM : forall A B C:set, A <> Empty -> B <> Empty -> C <> Empty -> forall P:set -> set -> prop, (exists f :e B :^: A, exists g :e C :^: A, P f g) <-> exists h:set -> set, (forall x :e A, h x :e B * C) /\ P (fun x :e A => h x 0) (fun x :e A => h x 1).
Admitted.

// HOL Light: pair.ml:372 / EXISTS_SWAP_FUN_THM
// Source hash: md5:9888ea217761e153767f9ad2147522e0
// Status: exact_native
Theorem EXISTS_SWAP_FUN_THM : forall A B C:set, A <> Empty -> B <> Empty -> C <> Empty -> forall P:set -> prop, (exists f :e C :^: B :^: A, P f) <-> exists f:set -> set -> set, (forall x :e B, forall x0 :e A, f x x0 :e C) /\ P (fun a :e A => fun b :e B => f b a).
Admitted.

