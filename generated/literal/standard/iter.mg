// hol2mg literal statements (private): shard iter of profile standard.  Each theorem is the literal
...
Theorem hlt_ORDER_EXISTENCE_CARD : forall A:set, A <> Empty -> forall R1 :e 2 :^: A :^: A, forall f :e A :^: A, forall z :e A, forall k :e omega, hl_FINITE (2 :^: A) (hl_GSPEC (2 :^: A) (fun GEN_PVAR_533 :e 2 :^: A => if exists n :e omega, hl_SETSPEC (2 :^: A) GEN_PVAR_533 (hl_IN omega n (hl_UNIV omega)) (R1 (hl_ITER A n f z)) = 1 then 1 else 0)) = 1 /\ (hl_le (hl_CARD (2 :^: A) (hl_GSPEC (2 :^: A) (fun GEN_PVAR_534 :e 2 :^: A => if exists n :e omega, hl_SETSPEC (2 :^: A) GEN_PVAR_534 (hl_IN omega n (hl_UNIV omega)) (R1 (hl_ITER A n f z)) = 1 then 1 else 0))) k = 1 /\ (R1 z z = 1 /\ ((forall x y :e A, R1 x y = 1 -> R1 y x = 1) /\ ((forall x y z1 :e A, R1 x y = 1 /\ R1 y z1 = 1 -> R1 x z1 = 1) /\ (forall x y :e A, R1 (f x) (f y) = 1 <-> R1 x y = 1))))) -> exists d :e omega, hl_lt (hl_NUMERAL hl_zero) d = 1 /\ (hl_le d k = 1 /\ forall n :e omega, R1 (hl_ITER A n f z) z = 1 <-> hl_num_divides d n = 1).
Admitted.

// HOL Light: Library/iter.ml:146 / ORDER_EXISTENCE_FINITE   (hash md5:476dd335351f1753fd4322f8dbfdb7c3)
Theorem hlt_ORDER_EXISTENCE_FINITE : forall A:set, A <> Empty -> forall R1 :e 2 :^: A :^: A, forall f :e A :^: A, forall z :e A, hl_FINITE (2 :^: A) (hl_GSPEC (2 :^: A) (fun GEN_PVAR_536 :e 2 :^: A => if exists n :e omega, hl_SETSPEC (2 :^: A) GEN_PVAR_536 (hl_IN omega n (hl_UNIV omega)) (R1 (hl_ITER A n f z)) = 1 then 1 else 0)) = 1 /\ (R1 z z = 1 /\ ((forall x y :e A, R1 x y = 1 -> R1 y x = 1) /\ ((forall x y z1 :e A, R1 x y = 1 /\ R1 y z1 = 1 -> R1 x z1 = 1) /\ (forall x y :e A, R1 (f x) (f y) = 1 <-> R1 x y = 1)))) -> exists d :e omega, hl_lt (hl_NUMERAL hl_zero) d = 1 /\ forall n :e omega, R1 (hl_ITER A n f z) z = 1 <-> hl_num_divides d n = 1.
Admitted.
