...
Admitted.
Theorem COND_RAND_bridge : (forall A B:set, A <> Empty -> B <> Empty -> forall b :e 2, forall f :e B :^: A, forall x y :e A, f (hl_COND A b x y) = hl_COND B b (f x) (f y)) -> (forall A B:set, B <> Empty -> forall b:prop, forall f:set -> set, (forall x :e A, f x :e B) -> forall x y :e A, f (if b then x else y) = if b then f x else f y).
exact (fun H__top A B HBne => (xm (A = Empty) (forall b:prop, forall f:set -> set, (forall x :e A, f x :e B) -> forall x y :e A, f (if b then x else y) = if b then f x else f y) (fun HAe => ((eq_sym_i A Empty HAe) (fun hl__u hl__v => forall b:prop, forall f:set -> set, (forall x :e hl__u, f x :e B) -> forall x y :e hl__u, f (if b then x else y) = if b then f x else f y) (fun b => (fun f => (fun H : forall x :e Empty, f x :e B => (forall_in_Empty (fun x => forall y :e Empty, f (if b then x else y) = if b then f x else f y))))))) (fun HAne => (imp_forall_bool (fun b => forall f :e B :^: A, forall x y :e A, f (hl_COND A b x y) = hl_COND B b (f x) (f y)) (fun b => forall f:set -> set, (forall x :e A, f x :e B) -> forall x y :e A, f (if b then x else y) = if b then f x else f y) (fun b => (imp_forall_fun (A) (B) (fun f => forall x y :e A, f (hl_COND A (if b then 1 else 0) x y) = hl_COND B (if b then 1 else 0) (f x) (f y)) (fun f => forall x y :e A, f (if b then x else y) = if b then f x else f y) (fun f Hfc => (imp_forall_in (A) (fun x => forall y :e A, hl_lam A f (hl_COND A (if b then 1 else 0) x y) = hl_COND B (if b then 1 else 0) (hl_lam A f x) (hl_lam A f y)) (fun x => forall y :e A, f (if b then x else y) = if b then f x else f y) (fun x Hx => (imp_forall_in (A) (fun y => hl_lam A f (hl_COND A (if b then 1 else 0) x y) = hl_COND B (if b then 1 else 0) (hl_lam A f x) (hl_lam A f y)) (fun y => f (if b then x else y) = if b then f x else f y) (fun y Hy => (imp_eq (hl_lam A f (hl_COND A (if b then 1 else 0) x y)) (f (if b then x else y)) (hl_COND B (if b then 1 else 0) (hl_lam A f x) (hl_lam A f y)) (if b then f x else f y) ((hl_COND_if (A) (if b then 1 else 0) (If_in_2 b) (b) (If_1_iff b) (x) Hx (y) Hy) (fun hl__u hl__v => hl_lam A f (hl_COND A (if b then 1 else 0) x y) = f hl__u) ((hl_lam_ap (A) f) (hl_COND A (if b then 1 else 0) x y) (setexp_ap (A) (A) (hl_COND A (if b then 1 else 0) x) (setexp_ap (A) (A :^: A) (hl_COND A (if b then 1 else 0)) (setexp_ap (2) (A :^: A :^: A) (hl_COND A) ((hl_COND_in) (A) HAne) (if b then 1 else 0) (If_in_2 b)) (x) Hx) (y) Hy))) (((hl_lam_ap (A) f) (y) Hy) (fun hl__u hl__v => hl_COND B (if b then 1 else 0) (hl_lam A f x) (hl_lam A f y) = if b then f x else hl__u) (((hl_lam_ap (A) f) (x) Hx) (fun hl__u hl__v => hl_COND B (if b then 1 else 0) (hl_lam A f x) (hl_lam A f y) = if b then hl__u else hl_lam A f y) (hl_COND_if (B) (if b then 1 else 0) (If_in_2 b) (b) (If_1_iff b) (hl_lam A f x) (setexp_ap (A) (B) (hl_lam A f) (hl_lam_Pi (A) (B) f Hfc) (x) Hx) (hl_lam A f y) (setexp_ap (A) (B) (hl_lam A f) (hl_lam_Pi (A) (B) f Hfc) (y) Hy))))))))))))) (H__top A B HAne HBne)))).
Qed.
...
exact (COND_RAND_bridge hlt_COND_RAND).
Admitted.
