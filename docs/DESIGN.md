...
equality congruence by motive replacement): **346 of 2 685 public theorems receive generated
...
`natively_proved: true` (346 on Core).  `tools/check_public.sh` now proof-checks these as
...
N3b (246 → 262): membership goals close *through subset hypotheses* (`(Hsub z pf)` — `Subq`
applies definitionally), and negation goals close as typed lambda proof terms
(`(fun H : p => <proof of False>)`) — together they finish DIFF, INTER, UNION, SUBSET,
SUBSET_TRANS, IN_GSPEC and the remaining `sets.ml` equalities that carry `forall s c= A`
binders.  An implication-lambda variant was tried and dropped: it blew the search space
(the synthesis phase must stay within minutes; it now costs ~9 min, and memoizing failed
subgoals is the next perf item).

N3c (262 → 284): a last-resort ex falso that *derives* the contradiction with the full
term prover instead of only pattern-matching hypothesis pairs (EMPTY_SUBSET, the EMPTY/
DISJOINT families); `setminusI`'s non-membership premise closes recursively; `set_ext`
also fires on a variable-variable equation when both sides carry subset evidence; and an
`<->`-concluded hypothesis can be *applied* — the matcher targets either side of the iff
...
N3f (298 → 301): existential witnesses are also *mined by matching* — the ∃-body is
matched against each hypothesis, so `P (choose_in A P)` yields the witness `choose_in A P`
(EXISTS_THM); and two more built-in premises, `Subq_omega_int` (int_of_num and friends
...
N4c (304 → 305, 337 → 338): substitution results are beta-normalized at witness and
instantiation sites (`(fun w => e) t` and `e[w:=t]` now compare equal, matching Megalodon's
conversion), and an or-goal of the exact shape `p \/ ~ p` closes with `exact (xm p)`
(EXCLUDED_MIDDLE).  SELECT_REFL-style goals (a lambda-valued predicate variable applied to
a choice term) would need genuine higher-order matching — deferred.

N4d-lite (305 → 306, 338 → 339): function-sorted existentials (`exists f:set -> set, ...`)
get constant-lambda witnesses over membership-hypothesis terms — with beta normalization in
place, `(fun w => e0) 0` and `e0` compare equal, closing one_RECURSION on both profiles.

Next (N4d): *derived equations generalized* — factor the application core so matching can
...
N5g (340 -> 343): the multiplication batch — `mul_nat_SR` with the `mul_nat`/`add_nat`
agreement bridges and closure, SNo units and distributivity — lands MULT_0 and both
ADD_DISTRIBs; MULT_CLAUSES' successor equations still need the full nat/SNo chain strung
together (next batch's target, with the synthesis phase now ~7 min a further reason to
move to memoization).

N5h/i (343 -> 346, phase 7m -> ~3m45): failed-subgoal *memoization* in `close_term`
(context-keyed, adepth-aware; identical outputs, half the time — and the Multivariate
pass fits its window again), plus two hand-bridged lemmas in `mglib/native/logic.mg` —
`mul_SNo_SR_omega`/`mul_SNo_SL_omega` (the nat/SNo recursion chain composed once, then
cited as builtins) — landing MULT_CLAUSES, MULT and MULT_SUC.
