...
First run on Core: **149 of 2 610 public theorems receive generated native proofs, and the
whole set checks in 3 s against the native context alone** — God1 signature, native prelude
and the profile's `_definitions.mg`; no literal layer, no `hl_*` symbol anywhere.  The output
is committed under `generated/nativeproof/core/` and regenerated by `tools/update.sh`
...
`EQ_SYM`, `ISO_REFL`, the `sets.ml` tautologies).

Next (N2b): use previously translated public theorems and prelude/God1 lemmas as premises
(currently only the goal's own hypotheses are used), equality congruence (`f_equal` motives),
numeric side conditions, and DAG-guided lemma selection from §22's recordings so that
non-tautological theorems acquire native proofs; then the `natively_proved` status and the
public-shard emission switch of §23.2 step 4.
