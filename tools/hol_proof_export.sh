...
# Usage: tools/hol_proof_export.sh <out.jsonl> [cap] [name1,name2,...] [forced1,forced2,...]
#   forced names are exported regardless of the cap (large leaves of many proofs)
set -e
HERE=$(cd "$(dirname "$0")/.." && pwd)
REC=${HOL_LIGHT_REC_DIR:-$HERE/../repos/hol-light-rec}
[ -f "$REC/fusion.ml" ] || "$HERE/tools/make_hol_rec_dir.sh"
out=$(readlink -f "$1"); cap=${2:-1000}; names=${3:-}; forced=${4:-}
mkdir -p "$(dirname "$out")"
only=$(python3 -c "import sys;print('[' + ';'.join('\"%s\"' % n for n in sys.argv[1].split(',') if n) + ']')" "$names")
force=$(python3 -c "import sys;print('[' + ';'.join('\"%s\"' % n for n in sys.argv[1].split(',') if n) + ']')" "$forced")
cd "$REC"
export HOLLIGHT_DIR=$REC HOL2MG=$HERE HOL2MG_PROFILE=core
{
  ...
  echo "Hol2mg_proof.export \"$out\" $cap $only $force;;"
  echo "exit 0;;"
