...
run() { ./bin/hol2mg translate --export "$exp" --sig mglib/God1.mgs --mappings "$maps" --out "$out" --literal-out "$lit" --profile "$prof" --srcindex generated/internal/$prof.srcindex.json --timeout 10 --known-props "$out/known_props.txt" "$@"; }
run
