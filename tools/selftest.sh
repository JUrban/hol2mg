...
make -s
S=$(mktemp -d)
run() { ./bin/hol2mg translate --export generated/internal/core.jsonl --sig mglib/God1.mgs --mappings mappings/core.json,mappings/lists.json --out "$1" --profile core --srcindex generated/internal/core.srcindex.json --timeout 10 --known-props generated/public/core/known_props.txt >/dev/null; }
run "$S/a"; run "$S/b"
cmp -s "$S/a/core.manifest.json" "$S/b/core.manifest.json" && echo "determinism: OK" || { echo "determinism: FAIL"; exit 1; }
for f in "$S/a"/*.mg; do cmp -s "$f" "$S/b/$(basename "$f")" || { echo "determinism: shard differs $(basename "$f")"; exit 1; }; done
